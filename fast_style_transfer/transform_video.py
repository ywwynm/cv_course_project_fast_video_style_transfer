...
# added by Zhang Qi
def transform_video_and_generate_frames(opts):
  if opts.no_disk:
    ...
    log.info('calling ffmpeg to get frames: starting at ' + str(time.time()))
    subprocess.call(" ".join(in_args), shell=True)
    log.info('frames got, stopped at ' + str(time.time()))
    ...
    log.info('starting style transfer at ' + str(time.time()))
    evaluate.ffwd(in_files, out_files, opts.checkpoint, device_t=opts.device,
                  batch_size=opts.batch_size)
    log.info('style transfer stopped at ' + str(time.time()))
    fr = 30  # wtf
    out_args = [
      'ffmpeg',
      '-framerate', str(fr),
    ...
    log.info('calling ffmpeg to join frames: starting at ' + str(time.time()))
    subprocess.call(" ".join(out_args), shell=True)
    log.info('join frames stopped at ' + str(time.time()))
    log.info('Video at: ' + opts.out)
...
def main():
    ...
    transform_video_and_generate_frames(opts)
